...
export const generateUUID = (): string => {
    ...
    let d2 =
        (typeof performance !== "undefined" && performance.now &&
            (performance.now() * 1000)) || 0; // Performance now to ensure randomness
...
export const addLogin = async (
    ...
    if (kv) {
        const loginIPs: { [key: string]: LoginIP } = JSON.parse(
            await kv.get("login-ips") || "{}",
        );
        ...
        await kv.put(
            ...
            JSON.stringify({
                ...loginIPs,
                [ip]: { time, success: successCount, count, type },
            }),
        );
        return loginIPs;
    }
